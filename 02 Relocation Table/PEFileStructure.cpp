...
PIMAGE_DATA_DIRECTORY fnGet_Directory_Address(IN LPVOID lpBuffer, IN int nIndex)
{
	...
	PIMAGE_DATA_DIRECTORY pImage_Data_Directory_Ret = NULL;

	if (lpBuffer == NULL)
	{
		if (__DEBUG)
		{
			printf("fnGet_Directory_Address: lpBuffer == NULL \r\n");
		}
		pImage_Data_Directory_Ret = NULL;
		goto RET;
	}
	bStatus = fnGet_PE_NT_Header_Address_By_FileBuffer(lpBuffer, (PE_NT_HEADER **)&pNT_Header);
	if (bStatus != TRUE)
	{
		if (__DEBUG)
		{
			printf("fnGet_Directory_Address: fnGet_PE_NT_Header_Address_By_FileBuffer 错误 \r\n");
		}
		pImage_Data_Directory_Ret = NULL;
		goto RET;
	}
	pImage_Data_Directory_Ret = (PIMAGE_DATA_DIRECTORY)&pNT_Header->OptionalHeader.DataDirectory[nIndex];

RET:
	return pImage_Data_Directory_Ret;
}
...
{
	bool bStatus = FALSE;
	PIMAGE_DATA_DIRECTORY pImage_Data_Directory_Relocation = NULL;
	PIMAGE_BASE_RELOCATION pImage_Base_Relocation = NULL;
	PIMAGE_SECTION_HEADER pCurrent_Section_Header = NULL;
	DWORD dwIndexSection = 0;
	DWORD dwRelocationNumber = 0;
	unsigned uCount = 0;
	pImage_Data_Directory_Relocation = fnGet_Directory_Address(lpBuffer, IMAGE_DIRECTORY_ENTRY_BASERELOC);
	if (pImage_Data_Directory_Relocation == NULL)
	{
		if (__DEBUG)
		{
			printf("fnPrint_Relocation_Table: pImage_Data_Directory_Relocation == NULL \r\n");
		}
		bStatus = FALSE;
		goto RET;
	}
	if (pImage_Data_Directory_Relocation->VirtualAddress == 0x0)
	{
		if (__DEBUG)
		{
			printf("fnPrint_Relocation_Table: pImage_Data_Directory_Relocation->VirtualAddress == 0x0, 不存在重定位表 \r\n");
		}
		bStatus = FALSE;
		goto RET;

	}
	pImage_Base_Relocation = (PIMAGE_BASE_RELOCATION)(fnRVA_Convert_FOA_By_FileBuffer(pImage_Data_Directory_Relocation->VirtualAddress, lpBuffer) + (DWORD)lpBuffer);
	if (pImage_Base_Relocation == NULL)
	{
		if (__DEBUG)
		{
			printf("fnPrint_Relocation_Table: pImage_Relocation == NULL \r\n");
		}
		bStatus = FALSE;
		goto RET;
	}
	while (pImage_Base_Relocation->SizeOfBlock != NULL || pImage_Base_Relocation->VirtualAddress != NULL)
	{
		pCurrent_Section_Header = fnGet_Section_Header_by_RVA(pImage_Base_Relocation->VirtualAddress, lpBuffer, &dwIndexSection);
		printf("************SectionIndex:%x, SectionName:%8s****************\r\n", dwIndexSection + 1, pCurrent_Section_Header->Name);
		dwRelocationNumber = (pImage_Base_Relocation->SizeOfBlock - 4 - 4) / 2;
		printf("该表中一共：%dd / %xh 项\r\n", dwRelocationNumber, dwRelocationNumber);
		for (uCount = 0; uCount < dwRelocationNumber; uCount++)
		{
			if ((((WORD *)((int)pImage_Base_Relocation + 8))[uCount] & 0x3000) == 0x3000)
			{
				printf("Index:%x, Type:3, RVA:%08x\r\n", uCount + 1,(((WORD *)((int)pImage_Base_Relocation + 8))[uCount] ^ 0x3000) + pImage_Base_Relocation->VirtualAddress) ;
			}
			else
			{
				printf("Index:%x, Type:不是有效项, RVA:%08x\r\n", uCount + 1,(((WORD *)((int)pImage_Base_Relocation + 8))[uCount] ^ 0x3000) + pImage_Base_Relocation->VirtualAddress) ;
			}
		
		}
		pImage_Base_Relocation = (PIMAGE_BASE_RELOCATION)((int)pImage_Base_Relocation + sizeof(char) * pImage_Base_Relocation->SizeOfBlock);
...
PIMAGE_SECTION_HEADER fnGet_Section_Header_by_RVA(IN DWORD RVA, IN LPVOID lpBuffer, OUT DWORD* pIndexOfSection)
{
	...
	if (lpBuffer == NULL)
	{
		if (__DEBUG)
		{
			printf("fnGet_Section_Header_by_RVA(): lpBuffer == NULL \r\n");
		}
		pCurrent_Section_Header = NULL;
		goto RET;
	}
	bStatus = fnGet_PE_NT_Header_Address_By_FileBuffer(lpBuffer, (PE_NT_HEADER **)&pNT_Header);
	if (bStatus != TRUE)
	{
		if (__DEBUG)
		{
			printf("fnGet_Section_Header_by_RVA(): fnGet_PE_NT_Header_Address_By_FileBuffer 出错 \r\n");
		}
		pCurrent_Section_Header = NULL;
		goto RET;
	}
	bStatus = fnGetPE_Image_Section_Header_Structure_Array((PE_NT_HEADER *)pNT_Header, (PE_IMAGE_SECTION_HEADER **)&pCurrent_Section_Header, &uNumberOfSection);
	for (uCount = 0; uCount < pNT_Header->FileHeader.NumberOfSections; uCount++)
	{
		if (pCurrent_Section_Header[uCount].VirtualAddress<= RVA && RVA < (pCurrent_Section_Header[uCount].VirtualAddress + (pCurrent_Section_Header[uCount].SizeOfRawData > pCurrent_Section_Header[uCount].Misc.VirtualSize ? pCurrent_Section_Header[uCount].SizeOfRawData : pCurrent_Section_Header[uCount].Misc.VirtualSize)))
		{
			pCurrent_Section_Header = &pCurrent_Section_Header[uCount];
			*pIndexOfSection = uCount;
		}
...
RET:
	return pCurrent_Section_Header;
